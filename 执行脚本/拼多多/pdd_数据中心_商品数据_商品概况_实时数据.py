...
from extra.extra_date import get_date_min_max, get_date
...
if __name__ == '__main__':
    ...
    site = '拼多多'
    shop_cookies, crawl_day_list = select_shop_date(table_name, site, shop_name_list, 0)
    ...
    for i in shop_cookies:
        ...
        res = Obj.goods_data__goods_general_situation_realtime()
        results = res.get("result", {}).get("todayList", {})
        print( results)
        items = []
        for result in results:
            ...
            item["key"] = f"{item['店铺名称']}_{item['统计日期']}_{item['统计时间']}"
            items.append(item)
        print(items)

        DBManager(db_config=db_config).update_insert_date(items, table_name, primary_key="key")
        logger.info(f"{shop_name}_{today}数据已入库")
        logger.info("-" * 100)
    logger.info(f"\n{'*' * 120}")
