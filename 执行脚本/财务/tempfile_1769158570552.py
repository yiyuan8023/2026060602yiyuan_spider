...
def find_tables_with_empty_headers(
...
def find_tables_with_duplicate_headers(
    results: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    """
    查找包含重复标题的表格
        ...
        with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
            for sheet_name in excel_file.sheet_names:
                ...
                if empty_indices:
                    ...
                    print(
                        f"已清理文件 {file_path} 中工作表 '{sheet_name}' 的空标题列，保留 {len(non_empty_columns)} 列"
                    )
                else:
                    # 如果没有空标题，直接复制原表
        ...
        with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
            for sheet_name in excel_file.sheet_names:
                ...
                if len(columns_to_keep) < len(headers):
                    ...
                    df_cleaned.to_excel(writer, sheet_name=sheet_name, index=False)
                    print(
                        f"已清理文件 {file_path} 中工作表 '{sheet_name}' 的重复标题列，保留 {len(columns_to_keep)} 列"
                    )
                else:
                    # 没有重复标题，直接复制原表
                    df.to_excel(writer, sheet_name=sheet_name, index=False)
...
def fix_all_files_with_duplicate_headers(
    results: List[Dict[str, Any]], base_folder: str
):
    """
    修复所有包含重复标题的文件
    """
...
def save_empty_headers_report(
    empty_headers_results: List[Dict[str, Any]], output_path: str
):
    """
...
def save_duplicate_headers_report(
    duplicate_headers_results: List[Dict[str, Any]], output_path: str
):
    """
...
def main():
    ...
    output_file = os.path.join(
        os.path.dirname(folder_path), f"抖音账单工作表标题分析_{timestamp}.xlsx"
    )

    # 生成空标题报告文件名
    empty_headers_output_file = os.path.join(
        os.path.dirname(folder_path), f"抖音账单空标题报告_{timestamp}.xlsx"
    )

    # 生成重复标题报告文件名
    duplicate_headers_output_file = os.path.join(
        os.path.dirname(folder_path), f"抖音账单重复标题报告_{timestamp}.xlsx"
    )
    ...
    # 保存重复标题报告
    save_duplicate_headers_report(
        duplicate_headers_results, duplicate_headers_output_file
    )
    ...
    if results:
        ...
        if empty_headers_results:
            ...
            for result in empty_headers_results:
                print(
                    f"  - 文件: {result['文件名称']}, 工作表: {result['工作表名称']}, "
