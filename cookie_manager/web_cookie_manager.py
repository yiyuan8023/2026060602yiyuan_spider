...
class WebCookieManager:
    def __init__(self, first_url, first_cookie_str, target_url):
        ...
        )  # or "firefox" or "webkit".   # 使用Chromium浏览器
        ...
        try:
            ...
            if locator.count() > 0:
                ...
                if elements:
                    ...
                    locator = self.target_page.locator(
                        f"xpath=//div[@class='live-space-item']"
                    )
                    if locator.count() > 0:
                        return True
                    return False
    ...
    def main(self):
        ...
        try:
            ...
            print(f"执行过程中出错: {e}")
            self.close_web()
            result = {"status": 2, "content": "登录失败"}

        if self.login_successfully():
            cookies = self.get_new_cookies()
            self.close_web()
            result = {"status": 1, "content": cookies}
