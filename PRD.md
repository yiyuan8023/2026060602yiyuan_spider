...
但赤兔、淘系直播中控台等二次业务 Cookie 派生能力仍保留在采集项目内，由对应 `API/` 模块负责。
...
- `cookie_manager/`: Cookie 字符串转换、取值和二次业务 Cookie 辅助能力。
...
- 当前采集项目只消费可用账号 Cookie，并允许在业务 API 层派生赤兔、直播中控台等二次业务 Cookie。
- 不在普通 `jobs/` 脚本里实现登录、验证码、浏览器兜底或 Cookie 写回流程。
- 二次业务 Cookie 的校验、派生和写回应留在对应 `API/` 模块，不放入普通 `jobs/` 编排脚本。
