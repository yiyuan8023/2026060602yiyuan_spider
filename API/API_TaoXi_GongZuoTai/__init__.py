...
- 文件用途：声明淘系商家工作台 API 包对外入口，集中暴露交易 API、订单详情 API 和订单详情解析函数。
- 业务范围：适用于调用 API_TaoXi_GongZuoTai 包的任务脚本、导入探针和后续商家工作台能力扩展。
- 依赖入口：使用 API_TaoXi_GongZuoTai_Trade.TaoXiGongZuoTaiTradeApi、API_TaoXi_GongZuoTai_OrderDetail.TaoXiGongZuoTaiOrderDetailApi 和 parser_order_discount_details.parse_order_discount_details。
