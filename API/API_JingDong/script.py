...
def spider_2_6_cn(gridData, d, shop_name, script):
    ...
    for i in data:
        ...
        item.update({
            ...
            "日期类型": "day",
            "店铺名称": shop_name,
            "唯一去重": f"{get_1970_days(d)}{item['商品ID']}{shop_name}" if script == "jd1_1_02_jdsz" else f"{get_1970_days(d)}{item['skuID']}{shop_name}"

        })
        if isinstance(item["是否为商品"], bool) and ((item["是否为商品"] and script == "jd1_1_02_jdsz") or (
                not item["是否为商品"] and script == "jd1_1_06_jdsz")):
...
def jd1_1_01_jdsz(**kwargs):
    ...
    JdSzTrade = JdSzTradeAPI(cookie=cookie, shop_name=shop_name,account_name=account_name)
    date_list = generate_dates(startDate, endDate)
    for d in date_list:
        sleep(random.uniform(WAIT_TIME_MIN, WAIT_TIME_MAX))
        logger.info(f"正在爬取”{shop_name}“ {d} 的 ”jd1_1_01_jdsz_交易_交易概况_全部渠道【商家版】“")
...
def jd1_1_09_jdsz(**kwargs):
    ...
    startDate = kwargs.get("startDate")
    endDate = kwargs.get("endDate")
    JdSzProduct = JdSzProductAPI(cookie=cookie, shop_name=shop_name,account_name=account_name)
    date_list = generate_dates(startDate, endDate)
    for d in date_list:
        sleep(random.uniform(WAIT_TIME_MIN, WAIT_TIME_MAX))
        logger.info(f"正在爬取”{shop_name}“ {d} 的 ”jd1_1_09_jdsz_商品_类目分析_行业类目【商家版】“")
        res_json = JdSzProduct.fetch_product_analysis__category_analysis(date=d, startDate=d, endDate=d)
        # print(res_json)
        data = res_json["body"]["data"]
        if data:
            ...
            logger.warning(f"”{shop_name}“ {d} 的 ”jd1_1_09_jdsz_商品_类目分析_行业类目【商家版】“ 无数据")
...
def jd1_1_07_jdsz(**kwargs):
    ...
    startDate = kwargs.get("startDate")
    endDate = kwargs.get("endDate")
    JdSzService = JdSzServiceAPI(cookie=cookie, shop_name=shop_name,account_name=account_name)
    date_list = generate_dates(startDate, endDate)
    for d in date_list:
        sleep(random.uniform(WAIT_TIME_MIN, WAIT_TIME_MAX))
        logger.info(f"正在爬取”{shop_name}“ {d} 的 ”jd1_1_07_jdsz_服务_服务分析_售后服务单量【商家版】“")
        res_json = JdSzService.fetch_service_analysis__after_sale_service(date=d, startDate=d, endDate=d)
...
def jd1_1_02_jdsz(**kwargs):
    ...
    for d in date_list:
        sleep(random.uniform(WAIT_TIME_MIN, WAIT_TIME_MAX))
        logger.info(f"正在爬取”{shop_name}“ {d} 的 ”jd1_1_02_jdsz_商品_商品明细_spu【商家版】“")
        res_json = JdSzProduct.fetch_product_analysis__product_detail(date=d, startDate=d, endDate=d)
...
def jd1_1_06_jdsz(**kwargs):
    ...
    for d in date_list:
        sleep(random.uniform(WAIT_TIME_MIN, WAIT_TIME_MAX))
        logger.info(f"正在爬取”{shop_name}“ {d} 的 ”jd1_1_06_jdsz_商品_商品明细_sku【商家版】“")
        res_json = JdSzProduct.fetch_product_analysis__product_detail(date=d, startDate=d, endDate=d, type="1")
        gridData = res_json['content']["gridData"]
...
def jd1_1_08_jdsz(**kwargs):
    ...
    startDate = kwargs.get("startDate")
    endDate = kwargs.get("endDate")
    JdSzReport = JdSzReportAPI(cookie=cookie, shop_name=shop_name,account_name=account_name)
    sleep(random.uniform(WAIT_TIME_MIN, WAIT_TIME_MAX))
    logger.info(f"正在爬取”{shop_name}“ {startDate}-{endDate} 的 ”jd1_1_08_jdsz_报表_新建报表_店铺【商家版】“")
    df = JdSzReport.fetch_report_analysis__my_report__excel(startDate=startDate, endDate=endDate)
    if df.empty:
        items = {}
    else:
...
def jd1_1_04_jdsz(**kwargs):
    ...
    startDate = kwargs.get("startDate")
    endDate = kwargs.get("endDate")
    JdSzCustom = JdSzCustomAPI(cookie=cookie, shop_name=shop_name,account_name=account_name)
    date_list = generate_dates(startDate, endDate)
    for d in date_list:
        sleep(random.uniform(WAIT_TIME_MIN, WAIT_TIME_MAX))
        logger.info(f"正在爬取”{shop_name}“ {d} 的 ”jd1_1_04_jdsz_客户_关注店铺用户概况_数据概览【商家版】“")
        res_json = JdSzCustom.fetch_fans_summary__data_summary(date=d, startDate=d, endDate=d)
...
def jd1_1_05_jdsz(**kwargs):
    ...
    for d in date_list:
        sleep(random.uniform(WAIT_TIME_MIN, WAIT_TIME_MAX))
        logger.info(f"正在爬取”{shop_name}“ {d} 的 ”jd1_1_05_jdsz_客户_品牌会员_会员概况_开卡会员【商家版】“")
        res_json = JdSzCustom.fetch_vip_summary__data_summary(date=d, startDate=d, endDate=d)
