...
- 文件用途：解析腾讯文档《线上线索分配明细》中的“信息分配明细（经销）”Sheet，校验表头并归一化日期字段。
- 业务范围：适用于腾讯文档线上线索信息分配明细经销数据入库字段，目标数据由任务脚本补充来源站点和文档信息。
- 依赖入口：使用 date_utils.get_date、date_utils.get_is_date。
- 验收方式：执行 py_compile；通过在线导出样本验证 17 列表头、日期字段归一化和有效行过滤结果。
- 注意事项：parser 不发起网络请求、不写数据库、不输出电话、地址等明细内容；不默认过滤日期为空行。
"""
...
DATE_FIELDS = {"分配时间", "成交时间", "反馈时间"}


def parse_online_lead_assignment_dealer_records(
    raw_records,
    *,
    source_site,
    source_url,
