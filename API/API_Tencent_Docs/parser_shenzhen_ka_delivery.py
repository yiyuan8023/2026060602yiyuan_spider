...
- 文件用途：解析腾讯文档《深圳KA壁挂炉发货对接》中的发货明细 Sheet，校验表头并归一化日期字段。
- 业务范围：适用于腾讯文档深圳KA壁挂炉发货明细入库字段，目标数据由任务脚本补充来源站点和文档信息。
- 依赖入口：使用 date_utils.get_date、date_utils.get_is_date。
- 验收方式：执行 py_compile；通过在线导出样本验证 19 列表头、订单日期、发货时间和有效行过滤结果。
...
def parse_shenzhen_ka_delivery_records(
    raw_records,
    *,
    source_site,
    source_url,
