            ...
            for sheet_name, items in items_dict.items():
                if sheet_name == "分载体流量来源":
                    for item in items:
                        ...
                        item["key"] = (
                            f"{item['店铺名称']}_{day}_day_{item['流量载体']}_"
                            f"{item['一级来源']}_{item['二级来源']}_{item['三级来源']}"
                        )
                    DBManager().update_insert_data(
                        items, db_table_name[2], primary_key="key"
                    )
                ...
                if sheet_name == "经营优势来源渠道":
                    for item in items:
                        ...
                        item["key"] = (
                            f"{item['店铺名称']}_{day}_day_{item['流量载体']}_{item['来源名称']}"
                        )
                    DBManager().update_insert_data(
                        items, db_table_name[1], primary_key="key"
                    )
                ...
                if sheet_name == "全店流量来源":
                    ...
                    DBManager().update_insert_data(
                        items, db_table_name[0], primary_key="key"
                    )
