...
- 文件用途：采集拼多多数据中心商品概况，按店铺和日期范围生成入库记录。
- 业务范围：适用于拼多多数据中心-商品数据-商品概况，当前脚本位于未启用目录，目标表和店铺范围以 TASK_CONFIG 为准。
- 依赖入口：调用 API.API_Pdd.PddGoodsApi 获取原始数据，使用 build_goods_general_items 解析入库数据，使用 select_shop_date 获取店铺和日期，使用 DBManager 入库。
...
from API.API_Pdd.parser_data_center import build_goods_general_items
...
if __name__ == "__main__":
    ...
    for shop_config in TASK_CONFIG["shops"]:
        ...
        for shop_cookie in shop_cookies:
            ...
            logger.info(f"正在采集【{shop_name}】{start_date} 至 {end_date} 拼多多商品概况")
            response_json, font_mapping = api.goods_general_situation(start_date, end_date)
            items = build_goods_general_items(response_json, font_mapping, shop_name, end_date)
            if not items:
                logger.warning(f"{shop_name},{start_date}-{end_date} 拼多多商品概况无可入库数据")
                continue

            with DBManager(db_config=db_config) as db_manager:
                db_manager.update_insert_data(items, table_name, primary_key="key")

            logger.info(f"{shop_name},{start_date}-{end_date} 拼多多商品概况已入库 {len(items)} 条")
            logger.info(f"\n{'-' * 100}")
