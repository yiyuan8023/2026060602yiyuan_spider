...
- 文件用途：自动执行拼多多售后工作台批量导出，按店铺级并发下载报表 Excel、清洗记录并写入既有售后单表。
- 业务范围：适用于林内拼多多售后工作台 -> 售后单导出自动化，默认复用数据库 cookie 视图并按整段日期区间一次导出。
- 依赖入口：调用 API.API_Pdd.API_Pdd_AfterSalesExport.PddAfterSalesExportApi，使用 parser_after_sales_export 规范化记录，使用 select_shop_date 获取店铺和日期，使用 DBManager 入库。
- 验收方式：修改后执行 py_compile；真实采集时先单店铺单区间验证 Cookie、导出任务、Excel 解析和售后编号主键入库。
- 注意事项：正式运行建议通过 run_job.py 启动；日志不得输出完整 Cookie、签名下载 URL 或报表原文。
"""
...
from API.API_Pdd.parser_after_sales_export import normalize_after_sales_export_records
...
def fetch_shop_report(
    ...
    """单店铺导出整段时间区间的售后单，供店铺级并发调用。"""
    shop_name = shop_cookie[0]
    cookie = shop_cookie[1] or shop_cookie[2]
    db_config = shop_config_by_name.get(shop_name, {}).get("db_config")
    api = PddAfterSalesExportApi(cookie, shop_name=shop_name)
    logger.info(
        f"正在采集 {shop_name} 拼多多售后工作台明细，"
        f"time_range={start_timestamp}-{end_timestamp}"
    )
    payload = {
        "startCreatedTime": start_timestamp,
        "endCreatedTime": end_timestamp,
        "pageNumber": 1,
        "pageSize": 10,
    }
    raw_items = api.export_after_sales_records(
        payload,
        timeout_seconds=TIMEOUT_SECONDS,
        poll_seconds=POLL_SECONDS,
    )
    items = normalize_after_sales_export_records(raw_items, shop_name=shop_name)
...
def write_shop_items(shop_result):
    ...
    if not items:
        logger.warning(f"{shop_name} 拼多多售后工作台明细无可入库数据")
        return
    ...
    logger.info(f"{shop_name} 拼多多售后工作台明细已入库 {len(items)} 条")
    logger.info("-" * 100)
...
if __name__ == "__main__":
    ...
    if not shop_cookies:
        logger.warning("拼多多售后工作台明细没有可执行店铺")
    else:
        worker_count = min(MAX_WORKERS, len(shop_cookies))
        logger.info(
            f"拼多多售后工作台明细店铺级并发开始，店铺数={len(shop_cookies)}，"
        ...
        with ThreadPoolExecutor(max_workers=worker_count) as executor:
            ...
            for future in as_completed(future_by_shop):
                ...
                try:
                    ...
                    logger.exception(f"{shop_name} 拼多多售后工作台明细并发采集失败")
                    raise
