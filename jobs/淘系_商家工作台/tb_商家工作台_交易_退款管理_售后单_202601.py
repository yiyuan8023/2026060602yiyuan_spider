...
开发说明：
- 作者：一元
- 创建时间：2026-06-12 18:15:00
- 最近修改：2026-06-12 18:15:00
- 文件用途：执行商家工作台交易退款管理售后单报表采集任务，负责组织店铺、日期、目标表、店铺级并发、店铺字段补充和入库流程。
- 业务范围：适用于淘系商家工作台交易退款管理售后单报表，当前默认采集脚本内配置店铺的近期数据。
- 依赖入口：使用 API.API_TaoXi_GongZuoTai.TaoXiGongZuoTaiRefundReportApi、extra.select_shop_date、database.DBManager、date_utils.get_min_max_timestamps 和 extra.logger_。
- 验收方式：修改后执行 py_compile；真实请求时用单店铺、单日期验证报表下载、解析行数、目标表和主键。
- 注意事项：退款报表脚本独立于已卖出宝贝订单报表脚本；不在脚本内记录真实 Cookie、数据库密码或签名下载 URL；未确认前不做大范围真实写库。
"""

from concurrent.futures import ThreadPoolExecutor, as_completed

from API.API_TaoXi_GongZuoTai import TaoXiGongZuoTaiRefundReportApi
...
TABLE_NAME = "tb_商家工作台_交易_退款管理_售后单_202601"
SITE = "生意参谋"
PRIMARY_KEY = "退款编号"
MAX_WORKERS = 3

SHOP_CONFIGS = [
    {"shop_name": "林内热水器旗舰店", "db_config": None, "recent_days": 3},
]


def fetch_shop_report(shop_cookie, shop_config_by_name, start_timestamp, end_timestamp):
    """单店铺退款售后单报表导出和下载，供店铺级并发调用。"""
    shop_name = shop_cookie[0]
    cookie = shop_cookie[1]
    db_config = shop_config_by_name.get(shop_name, {}).get("db_config")
    api = TaoXiGongZuoTaiRefundReportApi(cookie)
    items = api.list_export_refund_report(start_timestamp, end_timestamp)
    for item in items:
        item["店铺名称"] = shop_name
...
def write_shop_items(shop_result):
    ...
    if not items:
        logger.warning(f"{shop_name} 商家工作台退款售后单报表无可写入数据")
        return
...
if __name__ == "__main__":
    ...
    if not shop_cookies:
        logger.warning("商家工作台退款售后单报表没有可执行店铺")
    else:
        worker_count = min(MAX_WORKERS, len(shop_cookies))
        logger.info(
            f"商家工作台退款售后单报表店铺级并发开始，店铺数={len(shop_cookies)}，并发数={worker_count}"
        )
        with ThreadPoolExecutor(max_workers=worker_count) as executor:
            ...
            for future in as_completed(future_by_shop):
                ...
                try:
                    ...
                    logger.exception(f"{shop_name} 商家工作台退款售后单报表并发采集失败")
                    raise
    logger.info("*" * 100)
