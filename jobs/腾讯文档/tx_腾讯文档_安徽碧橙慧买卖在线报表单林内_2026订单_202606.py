...
- 文件用途：导出腾讯文档中的安徽碧橙慧买卖“2026订单”，补充来源信息后按文件ID和Sheet刷新写入目标表。
- 业务范围：适用于腾讯文档站点下“腾讯文档-一元”Cookie 对应账号可访问的《安徽碧橙&慧买卖在线报表单-林内》在线表格，入库 Sheet 为“2026订单”。
- 依赖入口：调用 API.API_Tencent_Docs.TencentDocsFileApi 导出腾讯文档 Excel，使用 parser_anhui_bicheng_huimaimai_2026_order 解析字段，使用 DBManager 全删全入，日志走 extra.logger_。
...
TASK_CONFIG = {
    # "table_name": "tx_腾讯文档_安徽碧橙慧买卖在线报表单林内_2026订单_202606",
    "table_name": "rinnai_分销ka对公_慧买卖_202508",
    "site": "腾讯文档",
    "document_url": "https://docs.qq.com/sheet/DZXZyaXhzb3RyZUdN?tab=puui9h",
    "file_id": "300000000$evrixsotreGM",
    "sheet_name": "2026订单",
    "shops": [
        {
            "shop_name": "腾讯文档-一元",
...
def build_items(
    ...
    validate_anhui_bicheng_huimaimai_2026_order_headers(raw_items)
    return parse_anhui_bicheng_huimaimai_2026_order_records(
        raw_items,
...
if __name__ == "__main__":
    ...
    for shop_config in TASK_CONFIG["shops"]:
        ...
        logger.info(
            f"正在采集【{shop_name}】腾讯文档安徽碧橙慧买卖2026订单，"
        ...
        if not raw_items:
            logger.warning(f"{shop_name} 腾讯文档安徽碧橙慧买卖2026订单为空，跳过入库")
            continue

        items = build_items(raw_items, site, document_url, file_info, sheet_name)
        if not items:
            logger.warning(f"{shop_name} 腾讯文档安徽碧橙慧买卖2026订单没有有效数据，跳过入库")
            continue
        ...
        logger.info(f"{shop_name} 腾讯文档安徽碧橙慧买卖2026订单已全量刷新入库，rows={len(items)}")
        logger.info(f"\n{'-' * 100}")
