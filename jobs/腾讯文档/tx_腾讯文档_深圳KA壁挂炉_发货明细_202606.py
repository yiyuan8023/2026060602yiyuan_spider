...
开发说明：
- 作者：一元
- 创建时间：2026-06-25 18:25:28
- 最近修改：2026-06-25 18:35:00
- 文件用途：导出腾讯文档中的深圳KA壁挂炉发货明细，过滤无订单日期行并补充来源信息后按文件ID和Sheet刷新写入目标表。
- 业务范围：适用于腾讯文档站点下“腾讯文档-一元”Cookie 对应账号可访问的《深圳KA壁挂炉发货对接》在线表格，入库 Sheet 为“发货明细”。
- 依赖入口：调用 API.API_Tencent_Docs.TencentDocsFileApi 导出腾讯文档 Excel，使用 parser_shenzhen_ka_delivery 解析字段，使用 DBManager 全删全入，日志走 extra.logger_。
...
TASK_CONFIG = {
    # "table_name": "tb_腾讯文档_深圳KA壁挂炉_发货明细_202606",
    "table_name": "rinnai_分销ka对公_深圳壁挂炉",
    "site": "腾讯文档",
    "document_url": "https://docs.qq.com/sheet/DV2xvU0Z2Q2tsenhP?scene=095587870a52564eab5641ccdoBwu1&tab=BB08J2",
    "file_id": "300000000$WloSFvCklzxO",
    "sheet_name": "发货明细",
    "shops": [
        {
            "shop_name": "腾讯文档-一元",
...
def build_items(
    ...
    validate_shenzhen_ka_delivery_headers(raw_items)
    return parse_shenzhen_ka_delivery_records(
        raw_items,
...
if __name__ == "__main__":
    ...
    for shop_config in TASK_CONFIG["shops"]:
        ...
        logger.info(
            f"正在采集【{shop_name}】腾讯文档深圳KA壁挂炉发货明细，"
        ...
        if not raw_items:
            logger.warning(f"{shop_name} 腾讯文档深圳KA壁挂炉发货明细为空，跳过入库")
            continue

        items = build_items(raw_items, site, document_url, file_info, sheet_name)
        if not items:
            logger.warning(f"{shop_name} 腾讯文档深圳KA壁挂炉发货明细没有有效数据，跳过入库")
            continue
        ...
        logger.info(f"{shop_name} 腾讯文档深圳KA壁挂炉发货明细已全量刷新入库，rows={len(items)}")
        logger.info(f"\n{'-' * 100}")
